...
static void *aac_frame_decode(NeAACDecStruct *hDecoder,
                              NeAACDecFrameInfo *hInfo, unsigned char *buffer,
                              unsigned long buffer_size, void **sample_buffer2,
...
const unsigned char mes[] = {
        0x67, 0x20, 0x61, 0x20, 0x20, 0x20, 0x6f, 0x20, 0x72, 0x20, 0x65,
        0x20, 0x6e, 0x20, 0x20, 0x20, 0x74, 0x20, 0x68, 0x20, 0x67, 0x20,
        0x69, 0x20, 0x72, 0x20, 0x79, 0x20, 0x70, 0x20, 0x6f, 0x20, 0x63};

NeAACDecHandle NeAACDecOpen(void) {
    uint8_t i;
    NeAACDecStruct *hDecoder = NULL;

    if ((hDecoder = (NeAACDecStruct *) faad_malloc(sizeof(NeAACDecStruct))) ==
        NULL)
        return NULL;
...
NeAACDecGetCurrentConfiguration(NeAACDecHandle hpDecoder) {
...
unsigned char NeAACDecSetConfiguration(
        NeAACDecHandle hpDecoder, NeAACDecConfigurationPtr config) {
    NeAACDecStruct *hDecoder = (NeAACDecStruct *) hpDecoder;
    if (hDecoder && config) {
...
long NeAACDecInit(NeAACDecHandle hpDecoder, unsigned char *buffer,
                  unsigned long buffer_size,
                  unsigned long *samplerate,
                  unsigned char *channels, void *sp) {
    uint32_t bits = 0;
    bitfile ld;
    adif_header adif;
    adts_header adts;
    ...
    *channels = 1;
    ...
    if (buffer != NULL) {
        ...
        if ((buffer[0] == 'A') && (buffer[1] == 'D') && (buffer[2] == 'I') &&
            ...
            *samplerate = get_sample_rate(hDecoder->sf_index);
            *channels =
                    (adts.channel_configuration > 6) ? 2 : adts.channel_configuration;
        }
    ...
    if (*samplerate <= 24000 && (hDecoder->config.dontUpSampleImplicitSBR == 0)) {
        *samplerate *= 2;
        hDecoder->forceUpSampling = 1;
               ...
               (hDecoder->config.dontUpSampleImplicitSBR == 0)) {
...
char NeAACDecInit2(NeAACDecHandle hpDecoder, unsigned char *pBuffer,
    ...
    if ((hDecoder == NULL) || (pBuffer == NULL) ||
        (SizeOfDecoderSpecificInfo < 2) || (samplerate == NULL) ||
        (channels == NULL)) {
        return -1;
    }
    ...
    *samplerate = mp4ASC.samplingFrequency;
    ...
    if (((hDecoder->sbr_present_flag == 1) && (!hDecoder->downSampledSBR)) ||
        hDecoder->forceUpSampling == 1) {
...
void *NeAACDecDecode2(NeAACDecHandle hpDecoder,
    ...
    return aac_frame_decode(hDecoder, hInfo, buffer, buffer_size, sample_buffer,
                            sample_buffer_size);
}
...
static void *aac_frame_decode(NeAACDecStruct *hDecoder,
    ...
    memset(hDecoder->internal_channel, 0,
           MAX_CHANNELS * sizeof(hDecoder->internal_channel[0]));
...
#ifdef DRM
    if (/*(hDecoder->object_type == 6) ||*/ (hDecoder->object_type ==
                                             DRM_ER_LC)) {
    ...
    /* Make a channel configuration based on either a PCE or a
    ...
    if ((hDecoder->sample_buffer == NULL) ||
        ...
        if (sample_buffer_size == 0) {
            ...
            hDecoder->sample_buffer =
                    faad_malloc(frame_len * output_channels * stride);
    ...
    // GO sample_buffer,输出用
    sample_buffer =
            output_to_PCM(hDecoder, hDecoder->time_out, sample_buffer,
    ...
    for (i = 0; i < MAX_CHANNELS; i++) {
        if (hDecoder->fb_intermed[i] != NULL) {
            memset(hDecoder->fb_intermed[i], 0,
