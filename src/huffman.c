...
hcb *hcb_table[] = {0, hcb1_1, hcb2_1, 0, hcb4_1, 0,
                    hcb6_1, 0, hcb8_1, 0, hcb10_1, hcb11_1};

hcb_2_quad *hcb_2_quad_table[] = {0, hcb1_2, hcb2_2, 0, hcb4_2, 0,
                                  0, 0, 0, 0, 0, 0};

hcb_2_pair *hcb_2_pair_table[] = {0, 0, 0, 0, 0, 0,
                                  hcb6_2, 0, hcb8_2, 0, hcb10_2, hcb11_2};
...
static INLINE int16_t huffman_getescape(bitfile *ld, int16_t sp) {
    ...
    for (i = 4;; i++) {  // 至少四位 11110，11110
        if (faad_get1bit(ld DEBUGVAR(1, 6, "huffman_getescape(): escape size")) ==
            0) {
            break;
        }
    }
...
static uint8_t huffman_2step_quad(uint8_t cb, bitfile *ld, int16_t *sp) {
    ...
    // 记录好当前的bit位置和码字大小即可？
    // 码字，先取 hcbN[cb]bit，至少可以获得码字的总长度
    // 只取，但是不移动

    // ！抓取这个起始的比特位置
    // ！然后加上offset
    // ！hcb_2_quad_table[cb][offset].bits
    ...
    // 1. 先获取hcbN[cb]，即可以得到offset和额外bit数
    // 2. hcbN[cb] + extra_bits 一定完整，有可能hcbN[cb]还多一些
    // 3. hcb_2_quad_table[cb][offset].bits则为真实的长度

    // 抓取这个起始的比特位置 和 延续长度 (hcb_2_quad_table[cb][offset].bits)
    if (extra_bits) {
        // 额外码字也取上
        /* we know for sure it's more than hcbN[cb] bits long */
        faad_flushbits(ld, hcbN[cb]);
        // 根据实际情况，直接加上extra_bits所表示的值
    ...
    if (cb == 1 || cb == 2) flush_temp_stego(ld->sp, cb, bits_cur, sp, 4);
    return 0;
}
...
static uint8_t huffman_2step_quad_sign(uint8_t cb, bitfile *ld, int16_t *sp) {
    ...
    // 直接修改符号位置即可
    huffman_sign_bits(ld, sp, QUAD_LEN, cb);
    return err;
}
...
static uint8_t huffman_2step_pair_sign(uint8_t cb, bitfile *ld, int16_t *sp) {
    ...
    // 后取符号
    huffman_sign_bits(ld, sp, PAIR_LEN, cb);
    return err;
}
...
static uint8_t huffman_binary_pair(uint8_t cb, bitfile *ld, int16_t *sp) {
    ...
    if (cb == 5) flush_temp_stego(ld->sp, cb, bits_cur, sp, 2);
    return 0;
}
...
static void vcb11_check_LAV(uint8_t cb, int16_t *sp) {
    static const uint16_t vcb11_LAV_tab[] = {16, 31, 47, 63, 95, 127,
                                             159, 191, 223, 255, 319, 383,
                                             511, 767, 1023, 2047};
    uint16_t max = 0;
...
uint8_t huffman_spectral_data(uint8_t cb, bitfile *ld, int16_t *sp) {
    switch (cb) {
        case 1:
        case 2:                                   // 27x+9y+3v+w+40
            return huffman_2step_quad(cb, ld, sp);  // 符号位直接在码字中 x
        case 3:
            return huffman_binary_quad_sign(cb, ld, sp);  // 符号单独出现 over
        case 4:
            return huffman_2step_quad_sign(cb, ld, sp);  // 符号单独出现 over
        case 5:
            return huffman_binary_pair(cb, ld, sp);  // 符号位直接在码字中 x 比较复杂
        case 6:                                    // 9x+y+40
            return huffman_2step_pair(cb, ld, sp);  // 符号位直接在码字中
        case 7:                                   /* binary search for data pairs */
        case 9:
            return huffman_binary_pair_sign(cb, ld, sp);  // 符号单独出现 over
        case 8: /* 2-step method for data pairs */
        case 10:
            return huffman_2step_pair_sign(cb, ld, sp);  // 符号单独出现 over
        case 12: {
            uint8_t err = huffman_2step_pair(11, ld, sp);  // 不应使用这个码书，跳过
        ...
        default:
            ...
            printf("Non existent codebook number, something went wrong : %d \n", cb);
            return 11;
    }
    return 0;
}
